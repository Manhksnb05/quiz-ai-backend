...
public class ExamService implements IExamService {
    ...
    public ExamSummaryDTO saveExtractedExam(SaveExamRequestDTO request) {
        ...
        if (request.getUserId() != null) {
            userRepository.findById(request.getUserId()).ifPresent(exam::setUser);
        }
    ...
    @Override
    // Lấy chi tiết đề bất kể Public/Private — dùng cho luyện đề cá nhân
    public ExamDetailPublicDTO getExamDetail(Long id) {
        ...
        DateTimeFormatter formatter = DateTimeFormatter.ofPattern("dd/MM/yyyy HH:mm");
        if (exam.getCreatedAt() != null) dto.setCreatedAt(exam.getCreatedAt().format(formatter));
        dto.setCreatorName(exam.getUser() != null && exam.getUser().getName() != null
                ? exam.getUser().getName() : "Người dùng ẩn danh");
        ...
        List<QuestionClientDTO> questionDTOs = exam.getQuestions().stream().map(q -> {
            ...
            qDto.setAnswer(q.getAnswer()); // Trả về đáp án đúng
            return qDto;
